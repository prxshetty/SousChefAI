...
const Particles: React.FC<ParticlesProps> = ({
    ...
    const circleParams = (): Circle => {
        // Create an exclusion zone in the center (40% of width/height)
        const centerExclusionX = canvasSize.current.w * 0.3
        const centerExclusionY = canvasSize.current.h * 0.3
    ...
    const drawCircle = (circle: Circle, update = false) => {
        if (context.current) {
            const { x, y, translateX, translateY, size, alpha, emoji } = circle
            context.current.translate(translateX, translateY)

            // Draw emoji with randomized size
            context.current.font = `${size * 8}px Arial`
            context.current.globalAlpha = alpha
            context.current.fillText(emoji, x, y)
